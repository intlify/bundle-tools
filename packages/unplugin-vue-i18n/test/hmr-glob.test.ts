...
import { normalizeResourcePath } from '../src/core/resource'
import vueI18n from '../src/vite'
import { isTestFramework } from './utils'
...
async function transformTarget(
  ...
  for (const mod of collectModules(server)) {
    if (mod.file && normalizeResourcePath(mod.file) === normalizeResourcePath(absFile)) {
...
describe.skipIf(!isTestFramework('vite'))('locale resource HMR', () => {
  const roots: string[] = []
  ...
  test('normalizeResourcePath matches Windows and posix separators', () => {
    expect(normalizeResourcePath('C:\\proj\\en.yml')).toBe(normalizeResourcePath('C:/proj/en.yml'))
  })
  ...
  async function changeAndAssert(opts: {
    ...
    server.watcher.emit('change', normalizeResourcePath(file))
