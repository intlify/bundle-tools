...
const TRANSLATION_FUNCTIONS_MAP: Record<
  (typeof TRANSLATION_FUNCTIONS)[number],
  string
> = {
  $t: 't: $t',
  $rt: 'rt: $rt',
...
export function autoDeclarePlugin(options: {
  sourcemap: boolean
}): UnpluginOptions {
  return {
    ...
    transform(code, id) {
      ...
      const scriptSetup = parseSFC(code, { sourceMap: false }).descriptor
        .scriptSetup
      if (!scriptSetup) {
        return
      }
      ...
      if (missingFunctionDeclarators.size > 0) {
        debug(
          `injecting ${Array.from(missingFunctionDeclarators).join(', ')} declaration to ${id}`
        )
